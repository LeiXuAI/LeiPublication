...
% This function is to execute LDA with different weight forms
%  to solve this form: Sb*P = lambda * Sw*P
% Input: 
%       X -- training instance matrix (N *d)
%       Y -- training label matrix (N*q)
...
%[W] = weight_estimation_overall_saliency(X, Y, weight_type);
...
% normalisation weights for each instance to fit the reference work
...
if reduced_dimension > min_rank
    reduced_dimension = min_rank;
end
